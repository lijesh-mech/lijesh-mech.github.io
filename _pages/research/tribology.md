...
.hero-image{min-height:360px;background:#f3f1f7}.hero-image img{display:block;width:100%;height:100%;min-height:360px;margin:0;object-fit:cover;cursor:zoom-in}
...
.validation{padding:25px;border-top:6px solid var(--g);background:radial-gradient(circle at top right,rgba(70,29,124,.10),transparent 34%),var(--w)}
.badge{display:inline-flex;margin-bottom:13px;padding:7px 10px;border-radius:999px;background:var(--ps);color:var(--p);font-size:10px;font-weight:900;letter-spacing:.08em;text-transform:uppercase}
.validation h2{margin:0 0 11px;color:var(--p);font-size:22px;font-weight:850}.validation>p{margin:0 0 14px;color:var(--t);font-size:13px;line-height:1.65}
.validation ul{display:flex;flex-direction:column;gap:9px;margin:0;padding:0;list-style:none}.validation li{display:grid;grid-template-columns:22px 1fr;gap:9px;color:var(--t);font-size:13px;line-height:1.48}
.check{display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;border-radius:50%;background:var(--p);color:#fff;font-weight:900}
.note{margin-top:15px!important;padding-top:13px;border-top:1px solid var(--b);color:var(--m)!important;font-size:11px!important;font-style:italic}
...
.tabs{display:flex;flex-wrap:wrap;gap:9px;margin-bottom:20px}.tab{padding:9px 13px;border:1px solid var(--b);border-radius:999px;background:var(--w);color:var(--p);font-size:11px;font-weight:850;cursor:pointer}.tab.active,.tab:hover{background:var(--p);border-color:var(--p);color:#fff}
...
<div class="trib-page">
  ...
  <div class="opening">
    ...
    <aside class="validation">
      ...
      <p>An independent comparative study evaluated five wear models, including Archard’s model, and reported that the DEG-theorem-based wear model provided the strongest overall performance by:</p>
      <ul>
      ...
      <p class="note">The full comparative-study citation can be added here when finalized.</p>
    </aside>
  </div>
